<template>
    <div class="flex flex-wrap h-screen font-mono text-base">
        <div class="w-64 flex flex-col gap-4 mt-6">
            <div class="flex justify-between w-full px-4">
                <router-link to="/dev" class="text-lg  ">Development</router-link>
            </div>
            ...
            <feature-required class="mx-2" feature="development" :allow-more-than-one="true"/>
        </div>
        <div class="bg-slate-900 h-full border-l-2 border-black flex flex-col" :style="collapsed ? 'width: 55px;':'width: 300px;'">
            <div id="tabs" class="flex w-full justify-between" :class="{ 'flex-wrap': !collapsed, 'flex-col': collapsed }">
                <div class="flex" :class="{ 'flex-wrap': !collapsed, 'flex-col': collapsed }">
                    <button @click="tab = 'files'" :class="[tab === 'files' ? 'bg-slate-800' : 'bg-slate-900']" class="py-2 px-3">
                    ...
                    <button @click="tab = 'settings'" :class="[tab === 'settings' ? 'bg-slate-800' : 'bg-slate-900']" class="py-2 px-3">
            ...
            <div v-if="!collapsed && ! $store.getters.loadingFiles" class="p-2 bg-slate-800 flex-grow max-h-screen overflow-y-scroll">
